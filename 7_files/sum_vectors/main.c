...
int main() {
    ...
    int n;
    fscanf(input_file_1, "%d", &n);  // Read the size of the vectors
    fscanf(input_file_2, "%d", &n);

    // Dynamically allocate memory for the vectors
    int *vector_1 = (int *)malloc(n * sizeof(int));
    int *vector_2 = (int *)malloc(n * sizeof(int));
    int *sum_vector = (int *)malloc(n * sizeof(int));
    ...
    fprintf(output_file, "%d\n", n);  // Write the size of the sum vector
    for (int i = 0; i < n; i++) {
